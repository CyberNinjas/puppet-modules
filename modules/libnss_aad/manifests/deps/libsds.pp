...
class libnss_aad::deps::libsds($version = '2.0.0') {

  exec {'clone':
    cwd     => '/tmp',
    path    => ['/usr/bin', '/bin'],
    command => 'git clone https://github.com/antirez/sds libsds',
    created => '/tmp/libsds'
  }
  ...
  $cflags = @("CFLAGS"/L)
      -fPIC -fstack-protector -std=c99 -pedantic -Wall -Werror
      -shared -o libsds.so.${version} -Wl,-soname=libsds.so.${version}
      | CFLAGS

  exec {'compile':
    cwd     => '/tmp/libsds',
    path    => ['/usr/bin', '/bin'],
    command => "gcc ${cflags} sds.c sds.h sdsalloc.h",
